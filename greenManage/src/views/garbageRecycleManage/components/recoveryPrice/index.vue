<template>
  ...
  <el-table :data="tableData" style="width: 100%; height: 555px;">
    <el-table-column prop="recyPricId" label="回收类型ID" />
    <el-table-column prop="wasteName" label="垃圾类型" />
  ...
  <el-dialog v-model="dialogVisible" title="价格类目修改" width="500">
    <el-form ref="ruleFormRef" style="max-width: 600px" :model="ruleForm" status-icon label-width="auto"
      class="demo-ruleForm">
      <el-form-item label="回收类型ID" prop="recyPricId">
        <el-input v-model="ruleForm.recyPricId" type="text" autocomplete="off" disabled />
      </el-form-item>
      <el-form-item label="垃圾类型" prop="wasteType">
        <el-input v-model="ruleForm.wasteName" type="text" autocomplete="off" />
      </el-form-item>
      <el-form-item label="垃圾价格" prop="price">
        <el-input v-model="ruleForm.price" type="text" autocomplete="off" />
      </el-form-item>
...
import { insert, deleteOne, update, selectAll, selectOne, selectByPage } from '../../../../api/recPri';
...
const deleteInfo = (index: number) => {
  deleteOne(tableData[index].recyPricId)
